...
- **Default from template file** — On first load (no saved design in this browser) the app fetches `kitchen-design-template.json` from the site root and uses it as the starting kitchen (spec + colours + time-of-day). Falls back to the built-in default if the file is missing or the page is opened over `file://` (where `fetch` is blocked). **Reset design** now restores this template. A saved/in-progress design in `localStorage` still takes precedence on open, so reloading never wipes work — use **Reset design** to (re)load the template.